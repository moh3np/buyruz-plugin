...
| دیباگ بروزرسانی | نیازمند تست محیطی | با فعال‌سازی «دیباگ و لاگ‌ها» و تیک بخش بروزرسانی باید فایل‌هایی مثل `logs/rfa-update-YYYY-MM-DD.log` ساخته شود و حاوی گام‌های کامل (درخواست‌ها، پاسخ‌ها، خطاها) باشد؛ خاموش/روشن کردن ماسک باید متن هدر Authorization را مطابق انتظار تغییر دهد. |

## یادداشت‌ها

- تست خودکار در این مخزن اجرا نشده است. برای پوشش کامل، سناریوهای بالا باید در محیط وردپرس (PHP 7.4+ / WP 5.8+) بررسی شوند.
- توصیه می‌شود پس از فعال‌سازی دیباگ، پوشهٔ `logs/` از نظر دسترسی نوشتن و پاکسازی خودکار (حذف فایل‌های قدیمی‌تر از مقدار تعیین‌شده) بررسی شود.
